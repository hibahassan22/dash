...
import { fetchTripById } from "../services/tripService.js";
...
function normalizeTripStatus(trip) {
  ...
  const map = { pending: "معلقة", suspended: "معلقة", completed: "تم", cancelled: "ملغية", progress: "قيد التنفيذ", in_progress: "قيد التنفيذ" };
  return map[raw] ?? raw;
}
...
export default function TripDetailsPage() {
  ...
  const fetchTrip = useCallback(async ({ silent = false } = {}) => {
    ...
    setError(null);
    try {
      const data = normalizeTrip(await fetchTripById(tripId));
      setTrip(data);
      ...
      {activeTab === "trip" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <PersonCard title="معلومات العميل" name={customerName} phone={customerPhone}
            onProfile={() => trip.customer_id && setShowCustomerProfile(true)} />
        </div>
      )}
